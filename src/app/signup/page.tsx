    ...
    <div className="flex min-h-dvh">
        ...
        <div className="relative flex flex-1 flex-col justify-between overflow-hidden bg-ink p-10 xl:p-12">
          ...
          <div className="relative">
            <Link href="/" className="inline-flex items-center gap-2.5">
              ...
              <span className="text-lg font-semibold tracking-tight text-paper">Relay</span>
            </Link>
          </div>
      ...
      <div className="flex flex-1 items-center justify-center px-6 py-12 sm:px-10 gradient-mesh">
        ...
        <div className="relative w-full max-w-sm space-y-8">
          ...
          <form onSubmit={signup} className="space-y-5 rounded-2xl border border-line/60 bg-surface-raised p-6">
            <div className="grid gap-1.5">
              ...
              <Input
                ...
                className={cn(orgName.trim().length > 0 && !orgValid && 'border-status-no')}
              />
              {!orgValid && orgName.trim().length > 0 && (
                <p className="text-xs text-status-no">At least 2 characters.</p>
              )}
            </div>
            ...
            <div className="grid gap-1.5">
              ...
              <Input
                ...
                className={cn(email.trim().length > 0 && !emailValid && 'border-status-no')}
              />
              {email.trim().length > 0 && !emailValid && (
                <p className="text-xs text-status-no">Enter a valid email.</p>
              )}
            </div>
            ...
            <div className="grid gap-1.5">
              ...
              <Input
                ...
                className={cn(password.length > 0 && !passwordValid && 'border-status-no')}
              />
              {password.length > 0 && !passwordValid && (
                <p className="text-xs text-status-no">{8 -password.length} more characters needed.</p>
              )}
            </div>

            {error && (
              <p className="text-sm text-status-no" role="alert">{error}</p>
            )}
