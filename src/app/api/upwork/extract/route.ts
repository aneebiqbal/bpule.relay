import { extractUpworkJob } from '@/lib/ai/upwork-extract'
...
export async function POST(request: Request) {
  ...
  if (!rawText) {
    return new Response(JSON.stringify({ error: 'Paste the job post first.' }), {
      status: 400,
      headers: { 'Content-Type': 'application/json' },
    })
  }
  ...
  return sseStream(async (emit) => {
    ...
    const extracted = await extractUpworkJob(rawText)
    emit({
      type: 'done',
      extracted,
