...
1. **Done — implemented the `intelligenceInputHash` reuse boundary (Phase 6).** Added `src/lib/intelligence-v2/input-hash.ts` (deterministic sha256 of normalized source text + pipeline version + score version — conservative whitespace-only normalization, does NOT attempt semantic canonicalization, which stays a separate Phase 5 concern), a new `intelligence_input_hash` column + index on `leads` (migration `20260926000000_intelligence_input_hash.sql`, **not yet applied to the live database — written and reviewed only**), a `findLeadByIntelligenceInputHash` store method (organization-scoped explicitly, not relying on RLS alone), and wired `reuseIfUnchanged`/`forceReanalyze` into `produceCanonicalIntelligence` and both call sites that invoke it (`/api/prospect/analyze`, `/api/leads/extract`). 14 tests on the hash function + 11 on orchestrator reuse semantics (exact duplicate, whitespace noise, meaningful change, version mismatch as a safety net even if the lookup is stale, tampered-hash defense, Force Reanalyze bypass, no-lookup-provided fallback). **Important scope note**: this closes the gap for *re-pasting* previously-analyzed text into analyze/extract — it does not change Lead Detail, which already never re-invokes the pipeline on open/refresh (verified: no `produceCanonicalIntelligence` call anywhere in the Lead Detail render path). `/api/inbound/analyze` (a third call site) was not wired — inbound replies are a different semantic case (a new message arriving, not a re-paste) and reusing there needs its own design decision, not blind copy-paste; flagged as open, not silently extended.
   - **Not yet done**: applying the migration to the live Supabase project (needs explicit go-ahead — see below), and the "concurrent identical requests must not create duplicate canonical runs" requirement beyond the existing 30s in-process `deduplicated()` coalescing, which does not hold across separate serverless instances. That's a real gap (documented, not fixed) — true concurrency safety would need a DB-level unique constraint or advisory lock, which is schema/infra work beyond this pass.
